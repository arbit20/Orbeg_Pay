    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between gap-4">
                ...
                <Link v-if="can.create" :href="route('clients.create')">
                    ...
                    <div class="border-b border-gray-200 p-4">
                        <form @submit.prevent="submitSearch" class="flex flex-col gap-3 sm:flex-row">
                            <input
                                ...
                                placeholder="Buscar por razon social, nombre comercial, documento o email"
                            />
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Ventas</th>
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="client in clients" :key="client.id" class="hover:bg-gray-50">
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                        <div class="font-medium">{{ client.document_type_label }}</div>
                                        <div class="text-gray-500">{{ client.document_number }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900">{{ client.business_name }}</div>
                                        <div v-if="client.trade_name" class="text-sm text-gray-500">{{ client.trade_name }}</div>
                                    </td>
