...
const filteredPrices = computed(() => {
    const prices = props.metal?.prices ?? [];
    return prices.filter((p) => p.currency === selectedCurrency.value);
});
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="price in filteredPrices" :key="price.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">
                                        {{ formatCurrency(price.price_per_gram, price.currency, 4, 4) }}
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                        {{ formatCurrency(price.price_per_troy_ounce, price.currency, 2, 2) }}
                                    </td>
                                ...
                                <tr v-if="filteredPrices.length === 0">
                                    <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-500">
                                        No hay precios registrados para este metal en {{ selectedCurrency }}.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
