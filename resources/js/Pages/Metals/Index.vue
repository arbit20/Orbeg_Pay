...
defineProps({
    ...
    currencies: { type: Array, default: () => ['USD', 'PEN'] },
});

const formatCurrency = (amount, currency, minFrac = 2, maxFrac = 2) => {
                                ...
                                <tr v-for="metal in metals" :key="metal.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                        <template v-if="metal.latest_price">
                                            {{ formatCurrency(metal.latest_price.price_per_gram, metal.latest_price.currency, 4, 4) }}
                                        </template>
                                        <span v-else class="text-gray-400">Sin precio</span>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                        <template v-if="metal.latest_price">
                                            {{ formatCurrency(metal.latest_price.price_per_troy_ounce, metal.latest_price.currency, 2, 2) }}
                                        </template>
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                        <Link
                                            :href="route('metals.show', metal.id) + `?currency=${selectedCurrency}`"
