...
const checkUsername = async () => {
    try {
        const response = await fetch(route('username.check'), {
            ...
            body: JSON.stringify({ username: form.username, user_id: props.user.id }),
        });
...
<template>
    <Head :title="`Editar Usuario: ${user.username || user.name}`" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
                <div v-if="!user.username" class="mb-4 rounded-md bg-amber-50 border border-amber-200 p-4">
                    <p class="text-sm font-medium text-amber-800">
                        Este usuario aún no tiene un nombre de usuario asignado. Por favor, asígnale uno para cumplir con el requisito de transacciones anónimas.
                    </p>
                </div>
                    ...
                    <div class="p-6">
                        <form @submit.prevent="submit" class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div class="md:col-span-2 flex items-center justify-end gap-4">
                                <Link :href="route('users.show', user.id)" class="text-sm text-gray-600 underline hover:text-gray-900">
                                    Cancelar
                                </Link>
