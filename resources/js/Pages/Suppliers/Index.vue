    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between gap-4">
                ...
                <Link v-if="can.create" :href="route('suppliers.create')">
                    <PrimaryButton>Nuevo Proveedor</PrimaryButton>
                </Link>
            </div>
        </template>
                    ...
                    <div class="border-b border-gray-200 p-4">
                        <form @submit.prevent="submitSearch" class="flex flex-col gap-3 sm:flex-row">
                            <input
                                ...
                                placeholder="Buscar por razon social, documento, email o banco"
                            />
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Razon social</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Contacto</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Banco</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Pago</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Estado</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Compras</th>
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="supplier in suppliers" :key="supplier.id" class="hover:bg-gray-50">
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                        <div class="font-medium">{{ supplier.document_type_label }}</div>
                                        <div class="text-gray-500">{{ supplier.document_number }}</div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                        {{ supplier.supplier_type_label }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900">{{ supplier.business_name }}</div>
                                        <div v-if="supplier.trade_name" class="text-sm text-gray-500">{{ supplier.trade_name }}</div>
                                    </td>
